...
"""
Collateral_DF = Collateral_df.iloc[0:10]
Prin_Start_Date = datetime.strptime('2018-5-7', '%Y-%m-%d')
Int_Start_Date=datetime.strptime('2018-5-7', '%Y-%m-%d')
Fee_Start_Date=datetime.strptime('2018-5-7', '%Y-%m-%d')
...
"""
StaticPool = pd.read_excel('Static_Pool.xlsx', sheetname = 'Sheet1', encoding = 'gb18030')
HazardRate = HazardRates(StaticPool)
def HazardRateFunction(t):
    N = math.floor(t/(1/12))
    return HazardRate[N]
vHazardRateFunction = np.vectorize(HazardRateFunction)
"""
...
"""
Integration,_  = scipy.integrate.quad(HazardRateFunction, 0, 0.5, HazardRate)
"""
